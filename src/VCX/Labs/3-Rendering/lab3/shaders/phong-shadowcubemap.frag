...
float Shadow(vec3 pos, vec3 lightPos) {
    ...
    closestDepth = texture(u_ShadowCubeMap, fragToLight).r;
...
vec3 Shade(vec3 lightIntensity, vec3 lightDir, vec3 normal, vec3 viewDir, vec3 diffuseColor, vec3 specularColor, float shininess) {
    ...
    vec3 res = vec3(0);
    if(!u_UseBlinn)
        res = diffuseColor*(lightIntensity * max(0,dot(normal, lightDir))) + specularColor*(lightIntensity * pow(max(0, dot(reflect(-lightDir, normal), viewDir)), shininess));
    else
        res = diffuseColor*(lightIntensity * max(0,dot(normal, lightDir))) + specularColor*(lightIntensity * pow(max(0, dot(normal,normalize(viewDir+lightDir))), shininess));
    return res;
}
